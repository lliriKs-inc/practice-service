...
| GET | `/api/v1/cohorts/:cohortId/progress` | ADMIN or approved STUDENT participant | 200 | Weekly cohort progress |
...
| POST | `/api/v1/me/applications/:applicationId/documents/:type/generate` | STUDENT | 200 | Generate DOCX |
...
Document enum для EAV/read/download/generate: `INDIVIDUAL_TASK`, `TITLE_PAGE`, `REVIEW`, `NOTICE`. Generate endpoint принимает тот же uppercase `DocumentType` в path parameter.
...
| `REVIEW` | `student_fio`, `group`, `review_activities`, `review_characteristic`, `review_employed`, `review_next_practice`, `review_employment_offer`, `review_suggestions`, `review_grade` | ФИО/группа — STUDENT; review fields — ADMIN |
| `NOTICE` | `student_fio`, `group`, `practice_topic` | — |

`practice_start`, `practice_end`, даты этапов индивидуального задания и `year` выводятся сервером из `Cohort.practice_start/practice_end`; клиент не сохраняет их в EAV. Генератор использует значения только выбранного `Document.type`, поэтому одноимённые поля другого документа не могут случайно подменить результат.
