...
`DELETE /cohorts/:cohortId` requires `ADMIN` and deletes only a `DRAFT` cohort. Cohort configuration (tracks, survey/questions, invitation and test task) is deleted with it. Rejected applications are deleted together with their answers, submissions, reports, documents and task data; their stored files are also cleaned up. A cohort with at least one `PENDING` or `APPROVED` application is rejected with `409 COHORT_HAS_APPLICATIONS`; a non-draft cohort is rejected with `409 COHORT_NOT_DRAFT`. File cleanup runs after the database transaction and the operation is audited.
