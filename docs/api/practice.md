...
Доступ: администратор либо студент с собственной `APPROVED` заявкой в явно указанной когорте. Такой студент по explicit toggle видит всех одобренных участников этой когорты; посторонний студент получает `APPLICATION_NOT_FOUND`. Данные других когорт не выдаются.
...
Генерация выполняется как мутация; `:type` — uppercase `DocumentType`:

```http
POST /me/applications/:applicationId/documents/:type/generate
```

Для `REVIEW` студент заполняет `student_fio` и `group`, а поля `review_*` заполняет ADMIN. Даты практики, даты этапов индивидуального задания и год сервер получает из когорты и подставляет в DOCX автоматически.
