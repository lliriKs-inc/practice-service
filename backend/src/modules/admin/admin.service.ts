...
// той же логикой, что и selectWorkingApplications (используется в Задачах и
// Документах — daily-task-progress-read.service.ts / getDocuments), но без
// отбрасывания остальных заявок: списку заявок в админке нужно видеть все,
// просто пометив, какая из них рабочая (выбрана студентом явно, либо
// единственная одобренная, либо выбрана по тому же fallback, что и в
// Задачах/Документах — самая ранняя, если практика уже началась, а выбор
// так и не сделан).
