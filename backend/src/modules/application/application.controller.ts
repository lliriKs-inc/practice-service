...
export async function updateStatus(req: Request, res: Response, next: NextFunction) { try { return res.json(await service.updateStatus(param(req, "cohortId"), param(req, "applicationId"), updateApplicationStatusSchema.parse(req.body))); } catch (error) { return next(error); } }
